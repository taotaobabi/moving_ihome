...
class SMSCodeHandler(BaseHandler):
	...
	def post(self):
		...
		try:
			ret = self.db.get("select count(*) counts from ih_user_profile where up_mobile=%(mobile)s",mobile=mobile)
			logging.debug(ret["counts"])
		except Exception as e:
			logging.error(e)
			return self.write(dict(errno=RET.DBERR, errmsg="查询出错"))
